...
.BAfilt <- function(d, fc, N) {
  ...
  A <- bandSparse(N, k = 0:d, diagonals = DiagA, symm = TRUE)
  B <- bandSparse(N, k = 0:d, diagonals = DiagB, symm = TRUE)
  list(A = A, B = B)
}
...
beads <- function(y, d = 1, fc = 0.01, r = 6,
  ...
  e   <- rep(1, N - 1)
  D1  <- bandSparse(N - 1, N, k = c(0, 1), diagonals = list(-e,  e))
  D2  <- bandSparse(N - 2, N, k = c(0, 1, 2),
  ...
  for (i in seq_len(Nit)) {
    Dxw    <- as.numeric(D %*% x)
    lam_d  <- w * wfun(Dxw)
    Lambda <- Diagonal(x = lam_d)

    k <- abs(x) > EPS0
    gamma[!k] <- (1 + r) / (4 * EPS0)
    gamma[ k] <- (1 + r) / (4 * abs(x[k]))
    Gamma <- Diagonal(x = gamma)
