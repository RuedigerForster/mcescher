...
//    phi = @(x) abs(x) - EPS1 * log(abs(x) + EPS1);
	...
	SparseQR<SparseMatrix<double>, COLAMDOrdering<int> > qr(AB.at(0));
...
void removeRow(Eigen::SparseMatrix<double>& matrix, unsigned int rowToRemove)
{
	...
	for (int k = 0; k < matrix.outerSize(); ++k) {
		for (SparseMatrix<double>::InnerIterator it(matrix, k); it; ++it) {
			if ((unsigned int)it.row() < rowToRemove)
				triplets.push_back(Triplet<double>(it.row(), it.col(), it.value()));
			else if ((unsigned int)it.row() > rowToRemove)
				triplets.push_back(Triplet<double>(it.row() - 1, it.col(), it.value()));
		}
	}
...
void removeColumn(Eigen::SparseMatrix<double>& matrix, unsigned int colToRemove)
{
	...
	for (int k = 0; k < matrix.outerSize(); ++k) {
		for (SparseMatrix<double>::InnerIterator it(matrix, k); it; ++it) {
			if ((unsigned int)it.col() < colToRemove)
				triplets.push_back(Triplet<double>(it.row(), it.col(), it.value()));
			else if ((unsigned int)it.col() > colToRemove)
				triplets.push_back(Triplet<double>(it.row(), it.col() - 1, it.value()));
		}
	}
	matrix.resize(numRows, numCols);
	matrix.setFromTriplets(triplets.begin(), triplets.end());
...
std::vector<std::vector<double>> beadsBaseline::beads(std::vector<double> y, int d, double fc, double positivityBias, double lam0, double lam1, double lam2)
{
	...
	for (int i = 0; i < Iter; i++)
	{
		...
		SparseQR<SparseMatrix<double>, COLAMDOrdering<int>> QR2(BTB + (AB.at(0).transpose() * M * AB.at(0)) );
		//xz2 = QR2.solve(d1);
