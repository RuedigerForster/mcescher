...
.ABfilt <- function(deg, fc, N, K = 1) {
  ...
  rows_P <- N - deg
  cols_P <- N
  P <- Matrix::bandSparse(rows_P, cols_P, k = 0:deg, diagonals = lapply(0:deg, function(i) rep(p[i + 1], rows_P)))
  ...
  Q <- Matrix::bandSparse(rows_P, cols_P, k = 0:deg, diagonals = lapply(0:deg, function(i) rep(q[i + 1], rows_P)))
  ...
  if (K <= deg) {
    ...
    rows_D <- N - K
    cols_D <- N
    D <- Matrix::bandSparse(rows_D, cols_D, k = 0:K, diagonals = lapply(0:K, function(i) rep(d[i + 1], rows_D)))
    ...
    rows_P1 <- N - deg
    cols_P1 <- N - K
    P1 <- Matrix::bandSparse(rows_P1, cols_P1, k = 0:(deg - K), diagonals = lapply(0:(deg - K), function(i) rep(p1[i + 1], rows_P1)))
    ...
    rows_B1 <- N - K2
    cols_B1 <- N
    B1 <- t(Matrix::bandSparse(rows_B1, cols_B1, k = 0:K2, diagonals = lapply(0:K2, function(i) rep(d[i + 1], rows_B1))))
    
    p1 <- pracma::deconv(p, d)$q
    
    rows_D1 <- N - deg
    cols_D1 <- N - K2
    D1 <- Matrix::bandSparse(rows_D1, cols_D1, k = 0:(deg - K2), diagonals = lapply(0:(deg - K2), function(i) rep(p1[i + 1], rows_D1)))
