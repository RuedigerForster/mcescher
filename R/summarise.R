...
# inside this function.  Inputs:
#   signal_mat    aligned signal matrix (n_samples  n_chroms)
#   baseline_mat  fitted SASS baseline matrix
#   RT            retention time vector (minutes)
...
# Merge detected peaks that fall within merge_window of a named peak's RT_ref.
# Called after find_peaks(), before integrate_peaks().
# Config peaks with merge_window > 0 trigger merging of all sub-peaks in
# [RT_ref - merge_window, RT_ref + merge_window] into a single peak spanning
# the outer lo_idx..hi_idx of the group, keeping the tallest apex.
.merge_peaks_by_config <- function(pk, RT, config) {
  if (nrow(pk) == 0L || is.null(config) || length(config$peaks) == 0L)
    return(pk)

  for (cp in config$peaks) {
    mw <- cp[["merge_window"]]
    if (is.null(mw) || is.na(mw) || mw <= 0) next

    in_win <- pk$RT >= (cp$RT_ref - mw) & pk$RT <= (cp$RT_ref + mw)
    if (sum(in_win) < 2L) next

    sub    <- pk[in_win, , drop = FALSE]
    best   <- which.max(sub$height)
    merged <- sub[best, ]
...
#' @param signal_mat_comp  Optional compressed signal matrix (block-averaged).
#'                         Speeds up the ensemble step when provided.
#' @param RT_comp          Retention time vector matching \code{signal_mat_comp}.
#' @param baseline_mat_comp  Optional compressed baseline matrix.
#' @param signal_presass_mat  Optional pre-SASS signal matrix (same dims as
#'   \code{signal_mat}) used to compute \code{u_sass}.
#' @param method_config   List returned by \code{read_method_config()}.
#'   Supplies peak names, area scaling, and per-peak ensemble overrides.
#'   If NULL the function runs without peak naming or area scaling.
...
#' \dontrun{
#' data(Grob_mix)
#' cfg <- read_method_config(
#'   system.file("extdata", "method.yaml", package = "mcescher")
#' )
#' # Grob_mix is a raw signal matrix; supply a flat zero baseline for a
#' # quick non-ensemble run:
#' RT  <- as.numeric(rownames(Grob_mix))
#' bl  <- matrix(0, nrow = nrow(Grob_mix), ncol = ncol(Grob_mix))
...
pipeline_summary <- function(signal_mat,
  ...
  # ---- 1b. Merge sub-peaks declared in config ---------------------------------
  if (!is.null(method_config))
    peaks_list <- lapply(peaks_list, .merge_peaks_by_config,
                         RT = RT, config = method_config)
      ...
      if (use_comp_ens) {
        tryCatch(
          .baseline_ensemble(
            ...
            RT_full          = RT,
            linear_interp    = if (!is.null(method_config))
                                 method_config$baseline_inhibit else NULL),
  ...
  # ---- 3b. Debug baseline plots --------------------------------------------
  if (run_ensemble && use_comp_ens && !is.null(method_config)) {
    for (cp in method_config$peaks) {
      if (!isTRUE(cp[["debug_baselines"]])) next
      rt_ref  <- cp$RT_ref
      pk_name <- cp$name
      zoom    <- 1.2   # minutes either side of RT_ref

      # Pick column with largest area for this peak
      peak_areas <- vapply(seq_len(n_chrom), function(j) {
        pk_j <- peaks_list[[j]]
        if (nrow(pk_j) == 0L) return(0)
        rows <- abs(pk_j$RT - rt_ref) <= cp$RT_window
        if (!any(rows)) return(0)
        max(pk_j$height[rows])
      }, 0.0)
      j_best <- which.max(peak_areas)
      if (peak_areas[j_best] == 0) next

      en_j <- ens_list[[j_best]]
      if (is.null(en_j) || is.null(en_j$baselines)) next

      idx_zoom <- RT_comp >= (rt_ref - zoom) & RT_comp <= (rt_ref + zoom)
      rt_z     <- RT_comp[idx_zoom]
      sig_z    <- signal_bc_comp[idx_zoom, j_best]
      bl_mat_z <- en_j$baselines[idx_zoom, , drop = FALSE]

      png_path <- sprintf("debug_baselines_%s.png",
                          gsub("[^A-Za-z0-9]", "_", pk_name))
      png(png_path, width = 1000, height = 500)
      y_lim <- range(c(sig_z, bl_mat_z), na.rm = TRUE)
      plot(rt_z, sig_z, type = "l", lwd = 2, col = "steelblue",
           ylim = y_lim, xlab = "RT (min)", ylab = "Signal (compressed)",
           main = sprintf("Ensemble baselines  %s  [%s]",
                          pk_name, col_names[j_best]))
      for (k in seq_len(ncol(bl_mat_z)))
        lines(rt_z, bl_mat_z[, k], col = adjustcolor("firebrick", 0.5), lwd = 1)
      w_j   <- en_j$weights
      bl_mn <- as.numeric(bl_mat_z %*% w_j)
      lines(rt_z, bl_mn, col = "darkgreen", lwd = 2)
      legend("topright",
             legend = c("Signal", "Ensemble baselines", "Weighted mean"),
             col = c("steelblue", "firebrick", "darkgreen"),
             lwd = c(2, 1, 2))
      dev.off()
      message(sprintf("  Debug plot saved: %s", png_path))
    }
  }
