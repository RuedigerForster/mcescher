...
#' Baseline correction using airPLS (adaptive iteratively reweighted PLS).
#'
#' @param X       Row matrix of spectra/chromatograms (m rows  n samples).
#' @param lambda  Smoothness penalty. Default 1e8.
#' @param order   Difference penalty order. Default 2.
#' @param wep     Weight exception proportion at endpoints. Default 0.1.
#' @param p       Asymmetry parameter at endpoints. Default 0.05.
#' @param itermax Maximum iterations. Default 20.
#' @return List: \code{$Xc} corrected matrix, \code{$Z} baseline matrix.
#' @export
airPLS <- function(X, lambda=1e8, order=2, wep=0.1, p=0.05, itermax=20) {
  ...
  # Construct difference matrix D of given order (sparse  avoids nn dense alloc)
  D <- diff(Matrix::Diagonal(n), differences = order)
