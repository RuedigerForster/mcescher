...
#' Estimate a chromatographic baseline using the FlatFit algorithm.
#'
#' @param y          Numeric signal vector.
#' @param smoothness Smoothness penalty scale (default 1e-3).
#' @param p          Savitzky-Golay window as fraction of signal length (default 0.05).
#' @return Numeric vector of baseline estimates, same length as \code{y}.
#' @export
flatfit_baseline <- function(y, smoothness = 1e-3, p = 0.05) {
  ...
  D  <- diff(Matrix::Diagonal(L), differences = 2L)   # (L-2) x L sparse matrix
