...
# calculate SNR:
# SNR[i] = max(abs(x[i]  median(x[w])) / sigma), abs(x[i]  x[i1]) / sigma
# outlier = SNR[i] > 2.5
#
...
.interpolate_outlier <- function(ind, arr) {
  ...
  fit.seg<-segmented::segmented(fit.glm)
...
#' LMV-RSA baseline estimation for chromatographic signals.
#'
#' @param signal   Numeric signal vector.
#' @param baseline Ignored (kept for compatibility). Default NULL.
#' @param crit     Outlier criterion (MAD multiples). Default 2.5.
#' @param span     Sliding window size in local-minimum samples. Default 50.
#' @param max_iter Maximum outlier-replacement iterations. Default 20.
#' @return Numeric vector of baseline estimates, same length as \code{signal}.
#' @export
lmv_rsa <- function(signal, baseline = NULL, crit = 2.5, span = 50, max_iter = 20) {
  ...
  # stage 2b: Detect outlier points in the rst-order derivative of x
