...
#' @param signal_full      Full-resolution signal vector (optional). When
#'   provided alongside \code{RT_full}, integration is performed at full
#'   resolution while baseline fitting uses the compressed signal.
#' @param RT_full          Retention time vector matching \code{signal_full}.
#' @param linear_interp    List of \code{c(lo, hi)} RT ranges in which the
#'   fitted baseline is replaced by linear interpolation (baseline-inhibit).
...
.baseline_ensemble <- function(signal, RT, peaks,
  ...
  cfg_results <- parallel::mclapply(seq_len(n_cfg), function(k) {
    if (verbose) message(sprintf("  [ensemble] %d/%d  %s", k, n_cfg, cfg_names[k]))
    bl_comp <- configs[[k]]$fun(signal)
    if (!is.null(linear_interp)) {
      for (reg in linear_interp) {
        idx  <- which(RT >= reg[1] & RT <= reg[2])
        if (!length(idx)) next
        i_lo <- max(1L, idx[1] - 1L)
        i_hi <- min(length(bl_comp), idx[length(idx)] + 1L)
        if (!is.na(bl_comp[i_lo]) && !is.na(bl_comp[i_hi]))
          bl_comp[idx] <- approx(c(RT[i_lo], RT[i_hi]),
                                 c(bl_comp[i_lo], bl_comp[i_hi]),
                                 xout = RT[idx])$y
      }
    }
...
#'   together) receive low weight.  Produces the consensus point estimate _p
#'   and a first-pass uncertainty u1_p (weighted SD).
#'
#' Step 2 -- Z-score penalty for locally unstable peaks:
#'   Z(p,k) = (a(p,k) - a_hat(p)) / u1(p) anchors deviations to the step-1
...
.two_step_consensus <- function(areas) {
  ...
  # Reference: Gaussian expected value = sqrt(2/)  0.798.
