...
#' Read and validate a method YAML configuration file.
#'
#' @param path Path to the method YAML file.  Use
#'   \code{system.file("extdata", "method.yaml", package = "mcescher")} to
#'   locate the bundled template.
#' @return A validated, type-coerced list with sections: method, inhibit,
#'   peaks, processing, denoising, denoising_segments, baseline_inhibit,
#'   baseline, detection, integration, area_scale.
#' @examples
...
read_method_config <- function(path) {
  ...
  # baseline_inhibit: optional list of [lo, hi] RT ranges replaced by linear interp
  if (!is.null(cfg$baseline_inhibit))
    cfg$baseline_inhibit <- lapply(cfg$baseline_inhibit,
                                   function(x) as.numeric(unlist(x)))

  # denoising_segments: optional per-segment parameter overrides
  if (!is.null(cfg$denoising_segments)) {
    cfg$denoising_segments <- lapply(cfg$denoising_segments, function(s) {
      list(
        rt_min = as.numeric(if (!is.null(s$rt_min)) s$rt_min else 0),
        rt_max = as.numeric(if (!is.null(s$rt_max)) s$rt_max else Inf),
        d      = as.integer(if (!is.null(s$d))   s$d   else cfg$denoising$d),
        fc     = as.numeric(if (!is.null(s$fc))  s$fc  else cfg$denoising$fc),
        K      = as.integer(if (!is.null(s$K))   s$K   else cfg$denoising$K),
        lam    = as.numeric(if (!is.null(s$lam)) s$lam else cfg$denoising$lam)
      )
    })
  }
